...
boost::dynamic_bitset<> Generator::getGamma(std::string message) {
	...
	while (length > 0) {
		// If the binary message is shorter than 224 characters, we must calculate the number of gamma fragments generated by the second stage of the generator
		int amount = 7;
		if (length < 224) amount = std::ceil(length / 32.0);
		std::bitset<32> lfsr_output = lfsr->generate();
		boost::dynamic_bitset<> lcg_output = lcg->generate(lfsr_output, amount);

		std::string temp;
		boost::to_string(lcg_output, temp);
		gamma_str += temp;
		length -= 32 * amount;
	}
