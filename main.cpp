...
int main(int argv, char** argc) {
	...
	case 1:
		{
			std::string message = getStringFromFile("input\\message.txt");
			...
			do {
				...
				std::cin >> digit_key;
			} while (digit_key < 0 || (digit_key > caesar.getAlphabet().length() - 1) || std::cin.fail());

			caesar.setDigitKey(digit_key);
			caesar.convertAlphabet();
			caesar.encrypt(message, "output\\encrypted_caesar.txt");
	...
	case 2:
		{
			...
			do {
				...
				std::cin >> columns;
			} while (columns < 0 || std::cin.fail());

			SimpleEncryptionTables tables(rows, columns);
			tables.encrypt(message, "output\\encrypted_tables.txt");
			tables.decrypt("output\\encrypted_tables.txt", "output\\decrypted_tables.txt");
		}
		break;
	case 3:
		{
			std::string message = getStringFromFile("input\\message_vigenere.txt");

			Vigenere vigenere("input\\alphabet_vigenere.txt", "input\\keyword_vigenere.txt");
			vigenere.encrypt(message, "output\\encrypted_vigenere.txt");
	...
	case 4:
		{
			...
			std::string message = getStringFromFile("input\\message_affine.txt");
			...
			} while (a < 0 || (a > affine.getAlphabet().length() - 1) || badGCD || std::cin.fail());
			...
			do {
				...
				std::cin >> b;
			} while (b < 0 || (b > affine.getAlphabet().length() - 1) || std::cin.fail());

			affine.setA(a);
			affine.setB(b);
			affine.convertAlphabet();
			affine.encrypt(message, "output\\encrypted_affine.txt");
